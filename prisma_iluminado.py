...
def figure():
    ...
    glBegin(GL_POLYGON)
    for i in range(0,N):
        x = raio * math.cos(i*angulo) -diamRotacao
        y = raio * math.sin(i*angulo) -diamRotacao
        pontosBase += [(x,y)]
        glVertex3f(x, y, 0.0)
    glEnd()
    ...
    glBegin(GL_POLYGON)
    for i in range(0, N):
        w = raio * math.cos(i*angulo) -diamRotacao
        z = raio * math.sin(i*angulo) -diamRotacao
        pontosTampa += [(w, z)]
        glVertex3f(w, z, H)
    glEnd()
    ...
    glBegin(GL_QUADS)
    for i in range(0, N):
        glNormal3fv(calculaNormalFace( (pontosBase[i][0], pontosBase[i][1],0.0), (-diamRotacao,-diamRotacao,H), (pontosBase[(i+1)%N][0], pontosBase[(i+1)%N][1],0.0)))
